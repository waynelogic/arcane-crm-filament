...
>
    <a
        ...
        @endif
        @class([
            'fi-sidebar-item-button group relative flex items-center justify-center gap-x-3 rounded-lg px-3 py-2.5 outline-none transition duration-150',
            'hover:bg-primary-200 focus-visible:bg-gray-100 dark:hover:bg-white/5 dark:focus-visible:bg-white/5' => filled($url),
            'bg-primary-500 dark:bg-primary-400/20' => $active,
        ])
    >
        @if (filled($icon) && ((! $subGrouped) || $sidebarCollapsible))
            <x-filament::icon
                ...
                @class([
                    'fi-sidebar-item-icon h-6 w-6 duration-150',
                    'text-gray-400 dark:text-gray-500' => ! $active,
                    'text-white group-hover:text-primary-600 dark:text-primary-400' => $active,
                ])
            />
        @endif
        ...
        @if ((blank($icon) && $grouped) || $subGrouped)
            ...
            >
                ...
                <div
                    @class([
                        'relative h-1.5 w-1.5 rounded-full duration-150',
